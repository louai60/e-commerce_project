...
WHERE v.product_id = p.id AND v.id = p.default_variant_id;

-- Step 3: Update products that have variants but no default_variant_id set
-- (use the first variant for each product)
UPDATE products p
SET
    sku = v.sku,
    price = v.price,
...
-- Step 4: Set default values for products without any variants
UPDATE products
SET
...
-- Step 5: Add NOT NULL constraints to required columns
ALTER TABLE products
ALTER COLUMN sku SET NOT NULL,
ALTER COLUMN price SET NOT NULL,
ALTER COLUMN inventory_qty SET NOT NULL;

-- Step 6: Add unique constraint to SKU
ALTER TABLE products
ADD CONSTRAINT products_sku_unique UNIQUE (sku);

-- Step 7: Add inventory_status column if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
...
-- Step 8: Update inventory_status based on inventory_qty
UPDATE products
...
-- Step 9: Add category_id column for direct category reference
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
